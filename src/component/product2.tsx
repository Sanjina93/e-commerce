...
export const Product2 = () => {
  ...
  return (
    <div className="flex justify-center sm:w-[1257px] md:w-[1355px] lg:w-[1455px] xl:w-full  h-[895px] bg-white">
      <div className="w-[658px] h-[799px] gap-2.5">
        ...
        <div className='w-[630px] h-[2px] bg-[#ECECEC]'></div>
          ...
          <div className="grid grid-cols-3 w-[609px] h-[364px] gap-[30px]">
            {filteredProducts.map((product) => (
              <div key={product.id} className="w-[183px] h-[324px]">
                <Link to={`/product2/${product.id}`} className='no-underline'>
                  <div className="w-[183px] h-[162px]">
                    <img className="w-40 items-center object-cover" src={product.image} alt='product' />
                  </div>
                  <div className="w-[183px] h-[100px] pt-6 pb-9 px-6 text-center text-black">
                ...
                <div className='flex w-[108px] h-[34px] px-6 py-1.5 gap-1 text-center tracking-[0.1px]'>
                  <p className="w-[52px] h-[24px] text-sm">{product.price}</p>
                  <h5 className="w-[52px] h-[24px] text-sm">{product.amount}</h5>
                  <button onClick={() => prodContext?.setIsOpen(true)} className='top-0'><FontAwesomeIcon onClick={() => {
                    ...
                    icon={faShoppingCart} className="text-[20px] text-[#23A6F0]" /></button>
                  <div className={`top-0 right-0 fixed w-96 h-full bg-white shadow-lg transition-transform duration-300 transform p-4 ${prodContext?.isOpen ? "translate-x-0" : "translate-x-full"}`}>
                        ...
                        <div className="mt-2 space-y-4">
                          {prodContext.cart.map((item) => (
                            <div key={item.id} className="flex items-center border-b pb-2 mx-3 text-center">
                              ...
                              <button className="bg-blue-400 ml-3 mr-1 w-9 h-7 rounded-lg" onClick={() => prodContext?.addToCart?.(item)}>
                  ...
                  <FontAwesomeIcon className="text-[20px] text-[#23A6F0]" onClick={() => { prodContext?.setFav(prodContext.fav + 1) }} icon={faHeart} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="">
        <div className='relative'>
          ...
          <h6 className='absolute top-36 left-9 w-[74px] h-[24px] font-bold text-[#737373]'>5 Items</h6>
        </div>
        <img
          className="w-[389px] h-[796px] mt-16 object-cover"
          src="../../images/shopcard/5.png"
          alt="Product"
        />
